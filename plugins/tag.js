...
await message.forwardMessage(target,message.quoted,{detectLinks: true,contextInfo: {mentionedJid: jids, mentionedEmoji: emojis.slice(0, jids.length)}});
}))
Module({pattern: 'forward ?(.*)',use: 'utility', fromMe: true, desc: "Forwards message"}, (async (message, match) => {
    ...
    let Jids = [...match[1]?.match(/[0-9]+(-[0-9]+|)(@g.us|@s.whatsapp.net)/g)] || [message.jid]
        for (let jid of Jids) {
      await message.forwardMessage(jid, message.quoted,{detectLinks: true, contextInfo: {mentionedEmoji: emojis.slice(0, Jids.length)}});
    }
}));
Module({pattern: 'send ?(.*)',use: 'utility', fromMe: true, desc: "Sends message"}, (async (message, match) => {
    ...
    let Jids = [...match[1]?.match(/[0-9]+(-[0-9]+|)(@g.us|@s.whatsapp.net)/g)] || [message.jid];
        for (let jid of Jids) {
     await message.forwardMessage(jid, message.quoted,{contextInfo:{isForwarded: false, mentionedEmoji: emojis.slice(0, Jids.length)}, detectLinks: true});
